...
/* Validação de NAÇÃO por B ou b em caso de nome começar por M ou m*/

CREATE OR REPLACE TRIGGER VALIDA_COUNTRY_TRIGGER
BEFORE INSERT OR UPDATE ON CUSTOMER
FOR EACH ROW
BEGIN
    IF (NOT (:NEW.COUNTRY) LIKE 'B%') AND
       (NOT (:NEW.COUNTRY) LIKE 'b%') AND
       (:NEW.FIRSTNAME LIKE 'M%' OR :NEW.FIRSTNAME LIKE 'm%') THEN
